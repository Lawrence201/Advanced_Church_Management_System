...
try {
    ...
    $required = ['transaction_id', 'date', 'service_type', 'project_name', 'amount_collected', 'collection_method', 'status'];
    ...
    $project_description = isset($data['project_description']) ? $conn->real_escape_string($data['project_description']) : null;
    ...
    // Update query
    $sql = "UPDATE project_offerings SET 
            date = '$date',
    ...
    if ($conn->query($sql)) {
        if ($conn->affected_rows > 0) {
            ...
            $response['message'] = 'Project offering updated successfully';
        } else {
